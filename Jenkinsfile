...
pipeline {
    ...
    options {
        // Configure an overall timeout for the build of 10 minutes.
    ...
    stages {
        ...
        stage('Build') {
            steps {
                echo 'Building'
                sh 'mvn -U -B -e clean install -DskipTests ${MVN_SHOW_TIMESTAMPS}'
            }
        }
        ...
        stage('Stable Tests') {
            steps {
                echo 'Running tests'
                sh 'mvn -U -B -e clean install ${MVN_SHOW_TIMESTAMPS}'
            }
            post {
                always {
